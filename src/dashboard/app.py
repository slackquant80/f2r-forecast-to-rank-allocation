...
current_mtd = live.get("current_mtd", {"available": False})
perf = doc["completed_performance"]
...
mtd_available = bool(current_mtd.get("available"))
mtd_value = pct(current_mtd["mtd_return"]) if mtd_available else "Unavailable"
mtd_sub = f"through {current_mtd['as_of_date']} · live net" if mtd_available else "Qualified current mark pending"
...
st.markdown(f'''<div class="health"><div class="cell"><div class="hl">Official signal</div><div class="hv">{official["signal_period"]}</div><div class="hs">Authoritative target</div></div><div class="cell"><div class="hl">Current MTD</div><div class="hv">{mtd_value}</div><div class="hs">{mtd_sub}</div></div><div class="cell"><div class="hl">Preview signal</div><div class="hv">{preview_period}</div><div class="hs">Provisional next state</div></div><div class="cell"><div class="hl">Market data through</div><div class="hv">{preview_cutoff}</div><div class="hs">Latest admissible common close</div></div><div class="cell"><div class="hl">Preview change</div><div class="hv">{transition["change_count"]} swap{plural}</div><div class="hs">{transition["summary"]}</div></div></div>''', unsafe_allow_html=True)
...
with tabs[0]:
    ...
    if mtd_available:
        st.markdown(f'<div class="public-note"><strong>Current MTD:</strong> {mtd_value} through {current_mtd["as_of_date"]}. The mark uses the Official portfolio on the governed execution clock with bilateral 10 bp/side cost at the locked {current_mtd["execution_date"]} rebalance close; Preview is excluded.</div>', unsafe_allow_html=True)
...
with tabs[1]:
    m = perf["metrics"]
    rm = perf.get("research_metrics", m)
    st.markdown('<div class="section-head"><div><div class="sk">Completed performance</div><div class="stitle">Performance & risk</div></div><div class="snote">Governed daily execution clock · calendar-month returns</div></div>', unsafe_allow_html=True)
    st.markdown(f'''<div class="metrics"><div class="metric"><div class="ml">CAGR</div><div class="mv">{pct(m["cagr"])}</div><div class="ms">{perf["coverage"]}</div></div><div class="metric"><div class="ml">Ann. volatility</div><div class="mv">{pct(m["ann_vol"])}</div><div class="ms">Daily · 252D</div></div><div class="metric"><div class="ml">Sharpe ratio</div><div class="mv">{m["sharpe_rf0"]:.2f}</div><div class="ms">RF=0 · daily</div></div><div class="metric"><div class="ml">Max drawdown</div><div class="mv">{pct(m["max_drawdown"])}</div><div class="ms">Completed path</div></div><div class="metric"><div class="ml">Research avg turnover</div><div class="mv">{pct(rm["avg_turnover"])}</div><div class="ms">Half-L1 · frozen research</div></div><div class="metric"><div class="ml">Positive months</div><div class="mv">{pct(m["positive_months"])}</div><div class="ms">Hit rate</div></div></div>''', unsafe_allow_html=True)
    ...
    dd_chart = alt.Chart(dd).mark_area(opacity=.75).encode(x=alt.X("date_dt:T", title=None, axis=alt.Axis(format="%Y", tickCount=9, labelAngle=0)), y=alt.Y("drawdown:Q", title="F2R drawdown", axis=alt.Axis(format="%")), tooltip=[alt.Tooltip("date:T", title="Date"), alt.Tooltip("drawdown:Q", format=".1%")])
    ...
    annual_table = pd.DataFrame(perf["calendar_annual_returns"]).copy()
    ...
    st.markdown(
        ...
        '<div class="stitle">Calendar-year annual returns</div></div>'
        '<div class="snote">Calendar-year aggregation from daily live accounting</div></div>',
        unsafe_allow_html=True,
    )
    render_table(annual_table, history=True)
    st.markdown(
        '<div class="public-note">Primary monthly and annual returns are aggregated from the governed daily execution-clock wealth path. '
        'Frozen research returns remain separate evidence and are not spliced into primary live performance.</div>',
        unsafe_allow_html=True,
    )
...
with tabs[2]:
    hm = history["metrics"]
    st.markdown('<div class="section-head"><div><div class="sk">Allocation history</div><div class="stitle">Official target path</div></div><div class="snote">Frozen research targets through 2026-06 + qualified operational Official targets · Preview excluded</div></div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-head"><div><div class="sk">Recent target path</div><div class="stitle">Latest 12 Official signal months</div></div><div class="snote">Newest first</div></div>', unsafe_allow_html=True)
    render_table(recent, history=True)
    with st.expander(f"Full {hm['months']}-month Official target history"):
...
with tabs[3]:
    ...
    render_table(spec)
    st.markdown('<div class="note">This interface is generated from an approved public snapshot and is intentionally limited to the current decision state, Official target history, completed performance, and research evidence needed to interpret F2R.</div>', unsafe_allow_html=True)
